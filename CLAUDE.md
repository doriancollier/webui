...
Run a single test file: `pnpm vitest run apps/server/src/services/session/__tests__/transcript-reader.test.ts`
...
- **`routes/commands.ts`** - Slash command listing via `runtimeRegistry.getDefault().getCommands()`, which delegates to per-CWD `CommandRegistryService` instances cached in the runtime
...
- **`services/runtimes/claude-code/claude-code-runtime.ts`** - Claude Code runtime implementing the `AgentRuntime` interface. Encapsulates all Claude Agent SDK interactions: session management (create/resume with 30-minute timeout), streaming messaging (delegated to `message-sender.ts`), transcript reading, file watching (session broadcaster), tool approval (interactive handlers), per-CWD command registries (Map-based LRU cache, max 50), and session locking. Replaces the former standalone `AgentManager` class. DI uses narrow port interfaces: `setMeshCore(AgentRegistryPort)` and `setRelay(RelayPort)`. `watchSession()` delegates to `SessionBroadcaster.registerCallback()` for callback-based session watching. Injects MCP tool servers via `setMcpServerFactory()`.
- **`services/runtimes/claude-code/message-sender.ts`** - Extracted `executeSdkQuery()` async generator. Contains the SDK query execution pipeline: boundary validation, agent manifest loading, tool filtering via `tool-filter.ts`, system prompt building via `context-builder.ts`, SDK streaming, and event mapping. Extracted from `ClaudeCodeRuntime.sendMessage()` for file size management.
...
- **`services/runtimes/claude-code/session-broadcaster.ts`** - Manages cross-client session synchronization. Watches JSONL transcript files via chokidar for changes (including CLI writes). Maintains SSE connections with passive clients via `registerClient()` and callback-based listeners via `registerCallback()` (used by `ClaudeCodeRuntime.watchSession()`). Broadcasts `sync_update` events when files change. Debounces rapid writes (100ms). Uses incremental byte-offset reading via `transcriptReader.readFromOffset()`. File watchers are shared between SSE and callback listeners; cleanup occurs when all listeners for a session are gone. Graceful shutdown closes all watchers, connections, and callbacks.
...
- **`services/runtimes/claude-code/command-registry.ts`** - Scans `.claude/commands/` for slash commands. Parses YAML frontmatter via gray-matter. Caches results; supports `forceRefresh`. Used by ClaudeCodeRuntime's `getCommands(forceRefresh?, cwd?)` method, which maintains a per-CWD Map cache of `CommandRegistryService` instances.
...
Clients can subscribe to session changes via a persistent SSE connection: `GET /api/sessions/:id/stream`. This provides real-time sync across multiple clients (including CLI changes). The route uses `runtime.watchSession()` which delegates to `SessionBroadcaster.registerCallback()`, supporting both SSE and callback-based listeners with shared file watchers.
...
**Obsidian plugin:** `CopilotView` computes `repoRoot = path.resolve(vaultPath, '..')` (vault is `workspace/`, repo root is its parent). This is passed to `ClaudeCodeRuntime(repoRoot)`.

Both paths are used by `ClaudeCodeRuntime` as the SDK's working directory. Command discovery (`.claude/commands/`) is handled internally by the runtime's per-CWD `CommandRegistryService` cache.
...
Tests live alongside source in `__tests__/` directories within each app and package (e.g., `apps/server/src/services/session/__tests__/transcript-reader.test.ts`).
