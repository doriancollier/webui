...
- We want to show _something_ for slash commands (not skip them entirely as we do now)
...
2. The _next_ user message (the expanded prompt) gets tagged with `messageType: { type: 'command', commandName: '/ideate', commandArgs: '...' }`
