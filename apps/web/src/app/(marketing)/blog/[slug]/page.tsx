    ...
    <div className="mx-auto max-w-6xl px-6 pt-32 pb-24">
      ...
      <div className="flex gap-12">
        <article className="min-w-0 max-w-3xl flex-1">
          ...
          <Link
            href="/blog"
            className="inline-flex items-center gap-1 font-mono text-2xs tracking-[0.04em] text-warm-gray-light hover:text-brand-orange transition-smooth mb-8"
          >
            ...
            {page.data.tags && page.data.tags.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {page.data.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-warm-gray/10 px-2 py-0.5 font-mono text-xs text-warm-gray-light"
                  >
                    {tag}
                  </span>
                ))}
          ...
          {(prevPost || nextPost) && (
            <nav
              className="mt-16 flex items-start justify-between gap-8 border-t border-warm-gray-light/30 pt-8"
              aria-label="Post navigation"
            >
              {prevPost ? (
                <Link href={prevPost.url} className="group flex flex-col gap-1">
                  <span className="font-mono text-2xs tracking-[0.08em] uppercase text-warm-gray-light group-hover:text-brand-orange transition-smooth">
                    ← Older
                  </span>
