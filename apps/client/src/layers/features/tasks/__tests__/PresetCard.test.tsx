...
import { TaskTemplateCard } from '../ui/TaskTemplateCard';
...
describe('TaskTemplateCard', () => {
  ...
  describe('selectable variant', () => {
    ...
    it('calls onSelect with the preset when clicked', () => {
      ...
      render(<TaskTemplateCard preset={PRESET} variant="selectable" onSelect={onSelect} />);
      fireEvent.click(screen.getByRole('button'));
    ...
    it('applies selection ring styles when selected=true', () => {
      const { container } = render(
        <TaskTemplateCard preset={PRESET} variant="selectable" selected={true} onSelect={vi.fn()} />
      );
      const btn = container.querySelector('button')!;
