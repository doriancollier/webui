...
/** Persistent indicator bar showing running background agents below the chat input. */
export function RunningAgentIndicator({ agents }: RunningAgentIndicatorProps) {
  ...
  const maxDurationSeconds = Math.max(...agents.map((a) => Math.round((a.durationMs ?? 0) / 1000)));
