...
function TextSettingRow({ extensionId, status }: { extensionId: string; status: SettingStatus }) {
...
function NumberSettingRow({ extensionId, status }: { extensionId: string; status: SettingStatus }) {
  const [value, setValue] = useState(String(status.value ?? ''));
  const [saving, setSaving] = useState(false);
