 ...
 */
function buildConnectionPath(ax: number, ay: number, bx: number, by: number, lag = 0): string {
  const mx = (ax + bx) / 2;
  const my = (ay + by) / 2;
...
function NodeCircle({
  ...
  // Stroke appearance
  const strokeColor = isAvailable ? 'var(--color-brand-orange)' : 'var(--color-warm-gray)';
  ...
  return (
    ...
    >
      ...
      {/* Inner orange accent */}
      <circle r={7} fill="var(--color-brand-orange)" opacity={isAvailable ? 0.8 : 0.3} />
...
export function DiagramV15({ modules }: { modules: SystemModule[] }) {
  ...
  const [positions, setPositions] = useState<NodePositions>(() =>
    Object.fromEntries(Object.entries(DEFAULT_POSITIONS).map(([id, pos]) => [id, { ...pos }]))
  );
  ...
  const handleSvgPointerUp = useCallback((e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (curPos && defPos && dist(curPos.x, curPos.y, defPos.x, defPos.y) < SNAP_RADIUS) {
  ...
  const handleReset = useCallback(() => {
    ...
    nodeIds.forEach((nodeId) => {
      ...
      animate(curPos.x, defPos.x, {
        ...RELEASE_SPRING,
        onUpdate: (v) =>
          setPositions((prev) => ({ ...prev, [nodeId]: { ...prev[nodeId]!, x: v } })),
        onComplete: done,
      });
      animate(curPos.y, defPos.y, {
        ...RELEASE_SPRING,
        onUpdate: (v) =>
          setPositions((prev) => ({ ...prev, [nodeId]: { ...prev[nodeId]!, y: v } })),
        onComplete: done,
      });
    });
  }, [isResetting]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {CONNECTIONS.map((conn) => {
            const isDraggingConn = dragState?.nodeId === conn.from || dragState?.nodeId === conn.to;
      ...
      <ExpandOverlay module={expandedModule} onClose={() => setExpandedModule(null)} />
