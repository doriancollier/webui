  ...
  return (
    <g>
      ...
      {isTransfer && lineColors.length > 1 && (
        <circle cx={x + outerR - 4} cy={y - outerR + 4} r={4} fill={lineColors[1]} />
      )}
      ...
      <text
        ...
        style={{
          fontSize: isTransfer ? '13px' : '11px',
          fontWeight: isTransfer ? 700 : 600,
...
/** Metro map architecture diagram — DorkOS modules as transit stations. */
export function DiagramV7({ modules }: { modules: SystemModule[] }) {
  ...
  const allStationIds = Array.from(new Set(METRO_LINES.flatMap((l) => l.stations)));
  ...
  return (
    <motion.svg
      ...
      aria-label="DorkOS metro map architecture diagram"
      initial="hidden"
      whileInView="visible"
      viewport={VIEWPORT}
      variants={STAGGER}
