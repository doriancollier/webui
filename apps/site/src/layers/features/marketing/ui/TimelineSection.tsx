...
export function TimelineSection() {
  ...
  const y1 = useSpring(useTransform(scrollYProgress, [BEAM_START, BEAM_END], [0, svgHeight]), {
    stiffness: 400,
    damping: 90,
  });
  const y2 = useSpring(
  ...
  const velocityGlow = useSpring(useTransform(scrollVelocity, [-0.5, 0, 0.5], [0.4, 0, 0.4]), {
    stiffness: 300,
    damping: 40,
  });
  ...
  return (
    <section
      ref={sectionRef}
      className="relative px-8 py-16 md:py-32"
      style={{
    ...
    >
      ...
      >
        ...
        <div className="relative" ref={timelineRef}>
          ...
          {svgHeight > 0 && (
            <div
              className="absolute top-0 bottom-0 left-[108px] hidden -translate-x-1/2 md:block"
              aria-hidden="true"
            >
            ...
            {timelineEntries.map((entry, entryIndex) => (
              ...
              >
                ...
                <div className="flex-1 space-y-3">
                  <p className="text-charcoal text-[16px] leading-[1.5] font-semibold tracking-[-0.01em] md:text-[17px]">
                    {entry.headline}
                  </p>
...
function TimelineDot({
  ...
  return (
    <motion.div
      className="relative h-2 w-2 shrink-0 rounded-full motion-reduce:scale-100 motion-reduce:opacity-100"
      style={{
        background: '#E85D04',
        scale,
        opacity,
