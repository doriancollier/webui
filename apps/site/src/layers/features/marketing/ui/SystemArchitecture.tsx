    ...
    <section id="system" className="bg-cream-tertiary px-8 py-32">
      <div className="mx-auto max-w-5xl">
        <motion.div initial="hidden" whileInView="visible" viewport={VIEWPORT} variants={STAGGER}>
          ...
          >
            DorkOS isn&apos;t a chat UI. It&apos;s an autonomous agent operating system — an engine,
            a life layer, and a mesh of coordinating agents.
          </motion.p>
        </motion.div>
        ...
        {grouped.map(({ group, label, modules: groupModules }) => (
          <div key={group} className="mb-10 last:mb-0">
            <motion.p
              ...
              viewport={VIEWPORT}
              variants={REVEAL}
              className="text-2xs text-warm-gray-light mx-auto mb-4 max-w-4xl font-mono tracking-[0.12em] uppercase"
            >
              {label}
            </motion.p>
            <motion.div
              className={`grid ${GROUP_COLS[group]} mx-auto max-w-4xl gap-6`}
              initial="hidden"
              whileInView="visible"
              viewport={VIEWPORT}
              variants={STAGGER}
