...
<Callout type="info">
DorkOS requires Node.js 18 or later. Check with `node --version`.
</Callout>
...
This starts both the Express server (default port 4242) and the Vite dev server (port 4241). Open [http://localhost:4241](http://localhost:4241) in your browser.
</Step>
</Steps>
...
<Callout type="warn">
Always use `pnpm` scripts or prefix commands with `dotenv --` to ensure `.env` is loaded. Bare `turbo` commands do not pick up environment variables.
</Callout>
...
<TypeTable type={{
  ...
  "pnpm smoke:integration": { type: "command", description: "Full integration test (server + API + client in Docker)" },
  "pnpm smoke:npm": { type: "command", description: "Integration test against published npm package" },
  "pnpm docker:build": { type: "command", description: "Build runnable Docker image from local code" },
