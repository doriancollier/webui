...
<TypeTable type={{
  ...
  "DORKOS_HOST": { type: "string", description: "Bind address. Set to 0.0.0.0 for Docker or any network-accessible deployment", default: "'localhost'" },
  "DORKOS_DEFAULT_CWD": { type: "string", description: "Default working directory", default: "Repo root" },
...
<Callout type="warn">
  Docker deployments must set `DORKOS_HOST=0.0.0.0`. Without it, the server binds to `localhost` inside the container and port forwarding will not work. The official `Dockerfile.run` sets this automatically.
</Callout>
