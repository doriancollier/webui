...
- **Telegram adapter** — Connects a Telegram bot to the bus via grammy. Inbound messages publish to `relay.human.telegram.{chatId}`. Outbound messages on matching subjects are delivered to the chat. Supports real-time streaming via `sendMessageDraft`.
