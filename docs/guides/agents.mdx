...
<Callout type="info">
  Tool group overrides layer on top of server-level feature flags. Disabling a tool group for an
  agent removes those tools from that agent's sessions only — other agents are unaffected.
</Callout>
