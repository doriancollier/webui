...
Configure how the agent shapes Claude's behavior. See the [Persona Injection](/docs/guides/persona) guide for details.

### Capabilities

Per-agent tool group toggles and Mesh metadata:

- **Tool Groups** — enable or disable MCP tool domains per agent (Relay tools, Mesh tools, Adapter tools, Pulse tools). Each toggle has three states: inherited (follows global config), enabled, or disabled. Changes take effect on the next session.
...
Claude sessions can query their own agent identity via the `get_current_agent` MCP tool. This is always available, regardless of whether Mesh is configured.
