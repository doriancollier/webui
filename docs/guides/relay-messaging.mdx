...
The trace span statuses are: `pending` (in flight), `delivered` (written to endpoint), `processed` (handler completed), `failed` (handler errored), and `dead_lettered` (rejected by budget or access control).
