...
When a directory has an agent with persona text configured and `personaEnabled` set to `true`, DorkOS injects two XML blocks into the system prompt:
...
The `<agent_identity>` block is always included when an agent exists — it gives Claude basic awareness of which agent it is. The `<agent_persona>` block is only included when persona text exists **and** `personaEnabled` is `true`.
...
Disabling the toggle is useful when you want to compare Claude's behavior with and without the persona — the text is preserved, injection stops.
