...
```bash
# Via the REST API
curl -X POST http://localhost:4242/api/mesh/agents/cross-namespace \
  -H 'Content-Type: application/json' \
  -d '{"sourceNamespace": "backend", "targetNamespace": "frontend"}'
```

This creates a Relay access rule permitting messages from `relay.agent.backend.>` to `relay.agent.frontend.>`. The access rules are bidirectional by default, so both namespaces can communicate.
