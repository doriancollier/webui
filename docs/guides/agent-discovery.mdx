...
Discovery scans your filesystem for directories that contain recognized agent markers. Mesh ships with four detection strategies:

- **claude-code** — `.claude/` directory present
- **cursor** — `.cursor/` directory or `.cursorrc` file
- **copilot** — GitHub Copilot configuration markers
- **dork-manifest** — `.dork/agent.json` file (auto-imports previously registered agents)
...
<Callout type="info">
When Relay is enabled, registering an agent automatically creates a Relay endpoint at `relay.agent.{namespace}.{agentId}`, making it immediately reachable for inter-agent messaging.
</Callout>
