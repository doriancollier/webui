...
<TypeTable type={{
  "thinking_delta": { type: "{ delta: string }", description: "Incremental extended thinking content (when extended thinking is enabled)" },
}} />
...
<TypeTable type={{
  ...
  "tool_progress": { type: "{ toolCallId: string, progress: string }", description: "Tool execution progress update (e.g., percentage complete)" },
}} />
...
<TypeTable type={{
  ...
  "done": { type: "{ lastMessageIds?: { user: string, assistant: string } }", description: "Message stream complete. Includes JSONL-assigned IDs when clientMessageId was sent in the request." },
