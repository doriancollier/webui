...
<Callout type="info">
  All installation methods require a Claude API key from the [Anthropic Console](https://console.anthropic.com/). Non-Docker methods also require [Node.js](https://nodejs.org/) 18 or later.
</Callout>
...
<Callout type="info">
  **CI/Automation:** Skip interactive prompts with `--no-prompt`:
  ```bash
...
<Callout type="info">
  For persistent data, custom ports, Docker Compose, and production hardening, see the [Docker deployment guide](/docs/self-hosting/docker). Pin a specific version for reproducible deployments: `ghcr.io/dork-labs/dorkos:0.12.0`.
</Callout>
...
Starts the Express API server and Vite dev server concurrently. The interface is available at [http://localhost:4241](http://localhost:4241) with hot module replacement.
